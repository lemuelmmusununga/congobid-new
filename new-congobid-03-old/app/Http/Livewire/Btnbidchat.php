...
class Btnbidchat extends Component
{
    ...
    public function mount(){
        $this->enchere = Enchere::where('date_debut',now()->format('Y-m-d'))->where('heure_debut','<=',now()->format('H:i'))->where('state',1)->first();
            ...
            # code...
            $this->pivot = Auth::user()->pivotBideurEnchere->last();
            $this->sanction= Sanction::where('enchere_id',$this->pivot->enchere_id ?? 0)->where('user_id',Auth::user()->id)->where('statut',1)->where('deleted_at',null)->first();
