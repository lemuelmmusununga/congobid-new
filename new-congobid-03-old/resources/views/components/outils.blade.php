    ...
    @if (Auth::user() && Auth::user()->role_id == 5)
        ...
        <div class="block-power d-flex justify-content-between align-items-center">
            ...
            <a href=""data-bs-toggle="modal" data-bs-target="#modalEnchere_{{ $article->id }}">
                ...
                <span>{{Auth::user()->pivotbideurenchere->where('enchere_id',$article->enchere->id)->first()->roi ?? 0}}</span>
            </a>
            <a href="" data-bs-toggle="modal" data-bs-target="#modalEnchere_{{ $article->id }}">
                <img src="{{asset('images/foudre.png')}}" alt="foudre">
                <span>{{Auth::user()->pivotbideurenchere->where('enchere_id',$article->enchere->id)->first()->foudre ?? 0}}</span>
            </a>
            <a href=""data-bs-toggle="modal" data-bs-target="#modalEnchere_{{ $article->id }}">
                <img src="{{asset('images/click.png')}}" alt="click">
                <span>{{Auth::user()->pivotbideurenchere->where('enchere_id',$article->enchere->id)->first()->clicks ?? 0}}</span>
            </a>
            <a href="" data-bs-toggle="modal" data-bs-target="#modalEnchere_{{ $article->id }}">

                <img src="{{asset('images/bouclier.png')}}" alt="bouclier">
                <span>{{Auth::user()->pivotbideurenchere->where('enchere_id',$article->enchere->id)->first()->bouclier ?? 0}}</span>
            </a>
            @else
